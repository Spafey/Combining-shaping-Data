...
Somewhat predictably for a non-parametric sample jammed into an OLS regression, no p-value in the regression model was greater than 0.85 (!), with relationships between data therefore indistinguishable from chance occurrence.

Data cleaning in Excel using quartiles and filters yielded a second, only slightly leptokurtic sample, but this failed to find significance as well . . . though some errors in data filtering as well as overly conservative pruning come analysis time will have contributed to much uncertainty to the model. on top of that, data cleaning using quartiles is basic & messy: Z-scores would probably be more efficient next time.   
